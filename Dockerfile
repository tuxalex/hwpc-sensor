...
RUN apt update && \
    apt install -y build-essential git clang-tidy cmake pkg-config libczmq-dev libsystemd-dev uuid-dev libpthread-stubs0-dev libpthreadpool-dev && \
...
RUN useradd -d /opt/powerapi -m powerapi && \
    apt update && \
    apt install -y libczmq4 libcap2-bin libpthreadpool0 && \
